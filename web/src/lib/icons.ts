// The glyph set, copied from the design system's `assets/icons/`.
//
// Lucide on a 24px grid. The system draws every icon through one component so the mark
// inherits `currentColor` and nothing else in the page has to know it is an SVG; these are
// the glyph bodies that component renders. Stroke width is set by the component at 1.5,
// which is the weight the design system specifies — the stock files ship at 2 and read as
// software chrome beside letterpress type.
//
// The system's domain vocabulary is what picks a glyph, never decoration: `map-pin` place,
// `landmark` civic, `users` people, `scroll-text` document, `archive` repository,
// `clock` chronology, `ruler` measure, `pen-line` corrections.
